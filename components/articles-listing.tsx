...
export function ArticlesListing({
  ...
  const unseenFromUrl = searchParams.get('unseen')
  ...
  // Determine initial unseen state: URL param takes precedence, then defaultFilters
  const initialShowUnseen = useMemo(() => {
    if (unseenFromUrl !== null) {
      return unseenFromUrl === 'true'
    }
    return filterConfig.defaultFilters?.showUnseen ?? false
  }, [unseenFromUrl, filterConfig.defaultFilters?.showUnseen])
  ...
  const {
    ...
    initialShowUnseen: initialShowUnseen,
    visitedSlugs: visitedArticles,
    initialPage,
    excludeHashtags: filterConfig.excludeHashtags,
    requireHashtags: filterConfig.requireHashtags,
  })
