...
3. **Explicit purge on publish** — a GitHub Action fires on Vercel's `deployment_status` success event and purges the whole Cloudflare zone **only when the deployed commits touched `articles/**` or `content/trends/**`** (a new Blog Article or Newsletter Issue). Daily News Article pushes do not purge — News is SuperAdmin-only and uncached. Purge-everything over purge-by-URL: publishes are rare, refill is a few hundred requests, and URL enumeration rots silently.
...
- **Purge on every push (`on: push` + paths filter)** — rejected: purging at push time refills Cloudflare from the *old* deployment while Vercel is still building; `deployment_status` success closes that gap.
